...
<div class="container">
                ...
                <select name="selectedNamemin" id="selectedNamemin" style="width: 800px;">
                    ...
                    while ($result = mysqli_fetch_assoc($sql)) : ?>
                        ...
                        if (empty($_POST['searchTerm']) || stripos($result['name'], $_POST['searchTerm']) !== false) :
                            ...
                            if ($_SESSION['selectedNamemin'] === $result['name']) {
                                echo 'selected';
                            } ?>>
                                <?php
                                echo $result['name'] . ' - ' . $result['category']; ?>
                            </option>
                        <?php
                        endif; ?>
                    <?php
    ...
    <div class="container">
        <canvas id="myChart4" width="600" height="400"
                data-min-prices="<?php
                echo htmlspecialchars(json_encode(array_column($result4, 'min_price')), ENT_QUOTES, 'UTF-8'); ?>"
                data-start-dates="<?php
                echo htmlspecialchars(json_encode(array_column($result4, 'start_date')), ENT_QUOTES, 'UTF-8'); ?>">
        </canvas>
    </div>
    data-url="update_chart_data.php?name=<?php
    echo urlencode(isset($_POST['selectedNamemin']) ? $_POST['selectedNamemin'] : $selectedNamemin); ?>"
